...
export interface DesktopStateSnapshot { todos: unknown[]; cyclePlans: unknown[]; }
...
interface ElectronRepositoryOptions { bridge: DesktopDataBridge; todos: Todo[]; cyclePlans: CyclePlan[]; }
...
export function createElectronRepositories(options: ElectronRepositoryOptions): ElectronRepositories {
  ...
  return {
    todoRepository: { loadTodos: () => structuredClone(serverSnapshot.todos), saveTodos: (todos) => save({ todos }) },
    cyclePlanRepository: {
      loadPlans: () => structuredClone(serverSnapshot.cyclePlans),
      savePlans: (cyclePlans) => save({ cyclePlans })
    },
    getWriteState: () => writeState,
    onWriteStateChanged: (listener) => { listeners.add(listener); listener(writeState); return () => listeners.delete(listener); },
