...
function entryPatch(before: CyclePlanEntry, after: CyclePlanEntry) {
  return {
    ...changed(before, after, ["date", "title", "contentSummary"]),
    ...(blocks(before.contentBlocks) === blocks(after.contentBlocks) ? {} : { contentBlocks: after.contentBlocks })
  };
}
...
function changed<T extends object>(before: T, after: T, keys: (keyof T)[]) {
  return Object.fromEntries(keys.filter((key) => before[key] !== after[key]).map((key) => [key, after[key]]));
}
